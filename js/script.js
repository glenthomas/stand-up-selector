...
Bird.obj = Bird.def(
  function() {
    ...
    fly: function(b) {
      if (this.ept) {
        this.accel.add(this.meet(this.ept, 0.05));
      } else {
    ...
    move: function() {
      ...
      // Apply damping when in formation mode and close to target
      if (this.ept) {
        var dist = this.pos.dst(this.ept);
        if (dist < 50) {
          // Strong damping when very close
          this.vel.scale(0.85);
        } else if (dist < 100) {
          // Medium damping when close
          this.vel.scale(0.92);
        }
      }
...
function draw() {
  ...
  // Generate formation positions based on text
  function generateFormationPositions(text) {
    var positions = [];
    var canvas = document.createElement('canvas');
    var ctx = canvas.getContext('2d');
    
    // Dynamically adjust canvas and font size based on text length
    var baseWidth = 1200;
    var baseHeight = 800;
    ...
    if (textLength > 8) {
      baseWidth = 1200 + (textLength - 8) * 80;
    }
    ...
    // Measure the actual text to ensure it fits
    var metrics = ctx.measureText(text);
    var textWidth = metrics.width;
    
    // If text is too wide, adjust canvas and redraw
    if (textWidth > baseWidth * 0.9) {
    ...
    // Sample pixels to find positions
  ...
  for (var i = 0; i < 200; i++) {
    ...
    _b.msp = 2; // Slower speed for continuous flight
  ...
  // Expose function to trigger formation with a new name
  window.triggerFormation = function(newName) {
    targetName = newName;
    formationMode = false;
    ...
    b.forEach(function(bird) {
      ...
      bird.pos.x = Math.random() * 800 - 400;
      bird.pos.y = Math.random() * 800 - 400;
      bird.pos.z = Math.random() * 800 - 400;
      bird.vel.x = Math.random() * 2 - 1;
      bird.vel.y = Math.random() * 2 - 1;
      bird.vel.z = Math.random() * 2 - 1;
  ...
  function drawFrame() {
    // Animate the flock center point to move around
    var time = Date.now() * 0.0003; // Slow time factor for smooth movement
    Bird.V.x = Math.sin(time) * 300;
    Bird.V.y = Math.cos(time * 0.7) * 200;
    
    // Only check elapsed time if formation has been triggered
    if (formationTriggered && startTime !== null) {
      var elapsed = Date.now() - startTime;
      
      // Switch to formation mode after delay
      if (!formationMode && elapsed > formationDelay) {
        formationMode = true;
        formationPositions = generateFormationPositions(targetName);
        
        // Assign each bird to a unique position
        b.forEach(function(bird, index) {
          bird._coll(false);
          bird.msp = 3; // Reduce max speed in formation
          bird.area = 0; // Turn off flocking behaviors
          if (index < formationPositions.length) {
            var target = formationPositions[index];
            bird.ept = new Bird.Vtr(target.x, target.y, target.z);
          } else {
            // Extra birds fly off screen
            // bird.ept = new Bird.Vtr(
...
(function() {
  ...
  var saveButton = document.getElementById('save-names');
  ...
  function saveNames() {
    ...
    teamNames = namesInput.value.split('\n').map(n => n.trim()).filter(n => n);
  ...
  pickButton.addEventListener('click', function() {
    ...
    if (selectedName) {
      // Update the global targetName variable
      window.targetName = selectedName.toUpperCase();
      
      // Trigger formation mode with the new name
      window.triggerFormation(selectedName.toUpperCase());
    }
  });
